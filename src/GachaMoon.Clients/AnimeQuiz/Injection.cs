...
public static class Injection
{
    ...
    public static IServiceCollection AddAnimeQuizClients(this IServiceCollection services)
    {
        services.AddSingleton<AnimeQuizApiClient>();
        services.AddSingleton<IAnimeQuizClient>(sp => sp.GetRequiredService<AnimeQuizApiClient>());

        services.AddHttpClient(ExternalClientType.AnimeQuizApi.ToString())
            .ConfigureHttpClient((sp, c) =>
            {
                c.DefaultRequestHeaders.TryAddWithoutValidation("X-Api-Key", "1234567890");
                c.BaseAddress = new Uri("http://anime-api:8085/");
            })
