...
public static class Injection
{
    ...
    public static IServiceCollection AddDiscordClients(this IServiceCollection services)
    {
        services.AddSingleton<DiscordApiClient>();
        services.AddSingleton<IDiscordApiClient>(sp => sp.GetRequiredService<DiscordApiClient>());

        services.AddHttpClient(ExternalClientType.DiscordApi.ToString())
            .ConfigureHttpClient((sp, c) =>
            {
                //c.BaseAddress = new Uri("https://discord.com/");
                c.DefaultRequestHeaders.TryAddWithoutValidation("X-Api-Key", "1234567890");
                c.BaseAddress = new Uri("http://94.159.98.101:8084/");
            })
